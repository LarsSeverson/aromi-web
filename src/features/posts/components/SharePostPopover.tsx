...
export interface SharePostPopoverProps {
  post: PostPreviewFragment
}

const SharePostPopover = (props: SharePostPopoverProps) => {
  const { post } = props
  const { id } = post
  ...
  return (
    <Popover.Root>
      ...
      >
        <PiShareFat
          className='group-hover/trigger:text-sinopia size-4.5'
        />
              ...
              >
                ...
                >
                  <BouncyButton
                    ...
                    onClick={(handleShareLink as () => void)}
                  >
                    <HiOutlineLink
                      size={20}
                    />
